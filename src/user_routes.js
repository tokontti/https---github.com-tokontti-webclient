...
router.get('/user/:id', authenticate, adminOnly, (req, res) => {
...
router.post('/user',authenticate, adminOnly, async (req, res) => {
...
// PATCH reitti käyttäjän salasanan päivittämiseen
router.patch('/user/:id', async (req, res) => {
    ...
    db.run("UPDATE user SET password = ? WHERE id = ?", [hashedPassword, id], (err) => {
        if (err) {
    ...
    db.get('SELECT id, password, role FROM user WHERE username = ?', [username], async (err, row) => {
        ...
        if (isAuthenticated) {
            ...
            db.serialize(() => {
                ...
                return res.json({ message: "Kirjautuminen onnistui", role: row.role });
            });
        } else {
            return res.status(400).send();
        }
